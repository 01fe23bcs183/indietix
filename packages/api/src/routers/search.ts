...
function parseNaturalLanguageQuery(query: string): SearchFilters {
  ...
  const priceRangeMatch = lowerQuery.match(
    /(?:₹|rs\.?|inr)?\s*(\d+)\s*[-–]\s*(?:₹|rs\.?|inr)?\s*(\d+)/i
  );
 ...
 */
function calculateFtsRank(
  event: { title: string; description: string; venue: string },
  query: string
): number {
  if (!query) return 0;
...
export const searchRouter = router({
  ...
  query: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const parsedFilters = input.q ? parseNaturalLanguageQuery(input.q) : {};
