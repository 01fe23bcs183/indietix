 ...
 */
const TIME_PATTERNS: Record<string, 'morning' | 'afternoon' | 'evening' | 'night'> = {
  morning: 'morning',
  am: 'morning',
  breakfast: 'morning',
...
function parsePrice(text: string): { min?: number; max?: number } {
  ...
  const underMatch = text.match(/(?:under|below|<|less than)\s*(?:rs\.?|₹|inr)?\s*(\d+)/i);
  if (underMatch) {
    result.max = parseInt(underMatch[1], 10);
  }
  
  // Match "above X" or "over X" or "> X"
  const aboveMatch = text.match(/(?:above|over|>|more than)\s*(?:rs\.?|₹|inr)?\s*(\d+)/i);
  if (aboveMatch) {
    result.min = parseInt(aboveMatch[1], 10);
  }
  
  // Match range "X-Y" or "X to Y" or "₹X–₹Y"
  const rangeMatch = text.match(/(?:rs\.?|₹|inr)?\s*(\d+)\s*(?:[-–—]|to)\s*(?:rs\.?|₹|inr)?\s*(\d+)/i);
  if (rangeMatch) {
...
export function parseNaturalLanguageQuery(query: string): SearchFilters {
  ...
  const skipWords = new Set([
    'near', 'in', 'at', 'on', 'for', 'the', 'a', 'an', 'and', 'or',
    'under', 'below', 'above', 'over', 'to', 'from', 'with',
    'today', 'tonight', 'tomorrow', 'weekend', 'week', 'next',
    'morning', 'afternoon', 'evening', 'night', 'am', 'pm',
    'rs', 'inr', '₹',
  ]);
  
  for (let i = 0; i < tokens.length; i++) {
    if (!usedTokens.has(i) && !skipWords.has(tokens[i]) && !/^\d+$/.test(tokens[i])) {
...
export function mergeFilters(
  ...
  return {
    ...parsed,
    ...Object.fromEntries(
      Object.entries(explicit).filter(([, v]) => v !== undefined && v !== null && v !== '')
    ),
  };
}
