...
test.describe("CMS Content Blocks", () => {
  test("homepage renders with default content when no blocks exist", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("homepage shows preview mode banner when in draft mode", async ({
    page,
  }) => {
    const token = Buffer.from(
      JSON.stringify({ key: "home.hero", exp: Date.now() + 3600000 })
    ).toString("base64");
  ...
  test("preview API rejects expired token", async ({ request }) => {
    const expiredToken = Buffer.from(
      JSON.stringify({ key: "home.hero", exp: Date.now() - 1000 })
    ).toString("base64");
...
test.describe("Help Center", () => {
  test("help page renders with correct title", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Find answers to your questions")
    ).toBeVisible();
  });
