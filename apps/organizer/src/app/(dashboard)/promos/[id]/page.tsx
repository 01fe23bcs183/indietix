...
export default function PromoDetailPage() {
  ...
  if (error || !promo) {
    return <div className="p-8 text-red-500">Error: {error?.message || "Promo code not found"}</div>;
  }
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white border rounded-lg p-6 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div>
            <h3 className="text-sm font-semibold text-gray-500 mb-2">Promo Code</h3>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">Value</h3>
              <p className="text-lg font-semibold">
                {promo.type === "PERCENT" ? `${promo.value}%` : formatINR(promo.value)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">Start Date</h3>
              <p className="text-lg">
                {promo.startAt 
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">End Date</h3>
              <p className="text-lg">
                {promo.endAt 
        ...
        <div className="border-t pt-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">Total Usage</h3>
              <p className="text-lg font-semibold">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">Per User Limit</h3>
              <p className="text-lg">
                {promo.perUserLimit || <span className="text-gray-400">Unlimited</span>}
              </p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-1">Min Purchase</h3>
              <p className="text-lg">
                {promo.minPrice ? formatINR(promo.minPrice) : <span className="text-gray-400">None</span>}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="space-y-4">
              ...
              {promo.applicableCategories && promo.applicableCategories.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {promo.applicableCategories.map((cat) => (
                    <span key={cat} className="px-3 py-1 bg-gray-100 rounded text-sm">
                      {cat}
                    </span>
                  ))}
                </div>
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-500 mb-2">Cities</h3>
              {promo.applicableCities && promo.applicableCities.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {promo.applicableCities.map((city) => (
                    <span key={city} className="px-3 py-1 bg-gray-100 rounded text-sm">
                      {city}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              {promo.applicableEvents && promo.applicableEvents.length > 0 ? (
                <p className="text-sm">{promo.applicableEvents.length} specific events</p>
              ) : (
