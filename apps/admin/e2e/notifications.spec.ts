...
test.describe("Admin Notification Preview", () => {
  ...
  test("should have template type selector", async ({ page }) => {
    ...
    await expect(selector.locator("option:has-text('booking_confirmed')")).toBeVisible();
    await expect(selector.locator("option:has-text('event_reminder_T24')")).toBeVisible();
  });
  ...
  test("should have channel selector", async ({ page }) => {
    ...
    await expect(channelSelector.locator("option:has-text('Email')")).toBeVisible();
    await expect(channelSelector.locator("option:has-text('SMS')")).toBeVisible();
    await expect(channelSelector.locator("option:has-text('Push')")).toBeVisible();
  });
